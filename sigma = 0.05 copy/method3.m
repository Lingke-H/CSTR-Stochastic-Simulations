...
% 如果你没有保存文件，直接把这段代码接在主程序后面运行也可以，去掉下面这一行 load
...
figure(6); clf;
% 使用 'FaceAlpha' 增加透明填充，让重叠部分也能看清
h_ev0 = histogram(max_open, 20, 'Normalization', 'pdf', ...
    'DisplayStyle', 'stairs', 'LineWidth', 2, 'EdgeColor', 'k', 'LineStyle', '--'); hold on;

h_ev1 = histogram(max_quad, 20, 'Normalization', 'pdf', ...
    'DisplayStyle', 'stairs', 'LineWidth', 2, 'EdgeColor', '#0072BD'); hold on;

h_ev2 = histogram(max_quart, 20, 'Normalization', 'pdf', ...
    'DisplayStyle', 'stairs', 'LineWidth', 2, 'EdgeColor', '#D95319'); hold on;

% 紫色 (Proposed II) 加粗，确保最显眼
h_ev3 = histogram(max_mixed, 20, 'Normalization', 'pdf', ...
    'DisplayStyle', 'stairs', 'LineWidth', 2.5, 'EdgeColor', '#7E2F8E'); 
...
legend([h_ev0, h_ev1, h_ev2, h_ev3], ...
    ...
    'Location', 'NorthWest'); % 改到左上角，通常那里是空的
xlim([0 0.4]);

%% === Figure 7: 时间演化 (拆分版 - 更清晰) ===
% 将图拆分为上下两部分，避免线条打架
figure(7); clf;
T_view = 50; 
