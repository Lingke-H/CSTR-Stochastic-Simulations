...
grid on;
title('Figure 6: Extreme Value Distribution (Smoothed)', 'FontSize', 12, 'FontWeight', 'bold');
...
ylim([min(x_open(1:N_view))-0.1, max(x_open(1:N_view))+0.1]);

% Subplot 2
subplot(2,1,2);
plot(t_vec, x_quad(1:N_view), '-', 'LineWidth', 0.5, 'Color', [0.7 0.8 1]); hold on; 
plot(t_vec, x_quart(1:N_view), '-', 'LineWidth', 1.0, 'Color', '#D95319');
plot(t_vec, x_mixed(1:N_view), '-', 'LineWidth', 1.0, 'Color', '#7E2F8E'); 
yline(0, 'k-', 'Alpha', 0.3);
...
grid on; xlim([0, t_vec(end)]); ylim([-0.2, 0.4]);

sgtitle('Figure 7: Time Domain Performance Comparison'); 
fprintf('所有绘图完成。\n');
